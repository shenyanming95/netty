...
public class HttpResponseStatus implements Comparable<HttpResponseStatus> {
    ...
    public static HttpResponseStatus parseLine(String line) {
        try {
            ...
            return space == -1 ? valueOf(parseInt(line)) : valueOf(parseInt(line.substring(0, space)), line.substring(space + 1));
        } catch (Exception e) {
    ...
    public String toString() {
        return new StringBuilder(reasonPhrase.length() + 4).append(codeAsText).append(' ').append(reasonPhrase).toString();
    }
