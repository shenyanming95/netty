...
public class HttpRequestEncoderInsertBenchmark extends AbstractMicrobenchmark {
    ...
    public ByteBuf oldEncoder() throws Exception {
        ...
        try {
            encoderOld.encodeInitialLine(buffer, new DefaultHttpRequest(HttpVersion.HTTP_1_1, HttpMethod.GET, uri));
            return buffer;
        } finally {
            buffer.release();
        }
    ...
    public ByteBuf newEncoder() throws Exception {
        ByteBuf buffer = Unpooled.buffer(100);
        try {
            encoderNew.encodeInitialLine(buffer, new DefaultHttpRequest(HttpVersion.HTTP_1_1, HttpMethod.GET, uri));
            return buffer;
        } finally {
            buffer.release();
        }
